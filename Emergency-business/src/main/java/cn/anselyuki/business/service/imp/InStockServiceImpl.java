...
import cn.anselyuki.business.converter.InStockConverter;
...
import cn.anselyuki.common.vo.business.InStockItemVO;
...
public class InStockServiceImpl implements InStockService {
    ...
    private final InStockConverter inStockConverter;
    ...
    @Autowired
    public InStockServiceImpl(InStockMapper inStockMapper, InStockConverter inStockConverter, ProductMapper productMapper, InStockInfoMapper inStockInfoMapper, ProductStockMapper productStockMapper, SupplierMapper supplierMapper) {
        this.inStockMapper = inStockMapper;
        this.inStockConverter = inStockConverter;
    ...
    public PageVO<InStockVO> findInStockList(Integer pageNum, Integer pageSize, InStockVO inStockVO) {
        ...
        List<InStockVO> inStockVOList = inStockConverter.converterToVOList(inStocks);
        PageInfo<InStock> inStockPageInfo = new PageInfo<>(inStocks);
        return new PageVO<>(inStockPageInfo.getTotal(), inStockVOList);
    }
        ...
        if (!CollectionUtils.isEmpty(inStockInfoList)) {
            for (InStockInfo inStockInfo : inStockInfoList) {
                ...
                if (!CollectionUtils.isEmpty(products)) {
                    ...
                    inStockDetailVO.getItemVOList().add(inStockItemVO);
                } else {
     ...
     * 物资入库
     *
     * @param inStockVO 入库参数对象
     */
    @Transactional(rollbackFor = {Exception.class}, noRollbackFor = {RuntimeException.class})
    @Override
    public void addIntoStock(InStockVO inStockVO) throws BusinessException {
        // 随机生成入库单号
        String inStockNum = UUID.randomUUID().toString().substring(0, 32).replace("-", "");
        // 记录该单的总数
        int itemNumber = 0;
        // 获取商品的明细
        ...
        if (!CollectionUtils.isEmpty(products)) {
            for (Object product : products) {
                LinkedHashMap<Object, Object> item = (LinkedHashMap) product;
                // 入库数量
